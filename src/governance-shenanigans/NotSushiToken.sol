...
contract NotSushiToken is ERC20("NotSushi", "NotSushi"), Ownable {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        /// @audit INTERESTING: Binary search of the value in the array
        uint32 lower = 0;
    ...
    function _delegate(address delegator, address delegatee)
        internal
    {
        // @audit-issue What happens if I delegate to myself?
        // will be 0x0 if there is no delegatee
        address currentDelegate = _delegates[delegator];
        // will be 500, won't change
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SUSHIs (not scaled);
        _delegates[delegator] = delegatee;

        emit DelegateChanged(delegator, currentDelegate, delegatee);
        // At first, 0x0 | attacker | 500
    ...
    function _moveDelegates(address srcRep, address dstRep, uint256 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            ...
            // at first, enters here with 0x0 | attacker | 500
            console.log("sender", msg.sender);
            if (dstRep != address(0)) {
                // increase new representative
                uint32 dstRepNum = numCheckpoints[dstRep]; // 0
                console.log("dstRepNum: ", dstRepNum);
                uint256 dstRepOld = dstRepNum > 0 ? checkpoints[dstRep][dstRepNum - 1].votes : 0;
                uint256 dstRepNew = dstRepOld.add(amount); // add 500 to 0
