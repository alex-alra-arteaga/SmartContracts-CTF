...
contract TastyStaking is Ownable {
    ...
    function _rewardPerToken(address _rewardsToken) internal view returns (uint256) {
        ...
        return
            rewardData[_rewardsToken].rewardPerTokenStored + (((_lastTimeRewardApplicable(rewardData[_rewardsToken].periodFinish) - rewardData[_rewardsToken].lastUpdateTime) * rewardData[_rewardsToken].rewardRate * 1e18) / totalSupply());
    }
    ...
    modifier updateReward(address _account) {
        {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                rewardData[token].rewardPerTokenStored = uint216(_rewardPerToken(token)); // 1e17
