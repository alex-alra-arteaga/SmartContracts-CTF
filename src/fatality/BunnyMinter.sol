...
contract BunnyMinter is Ownable {
    ...
    function mintForV2(
        ...
        // FEE EXECUTED, contract takes 1% of LP tokens
        ...
        // multiply per 5 the valueInBNB to get the amount of BUNNY to mint
        uint256 mintBunny = amountBunnyToMint(valueInBNB); // if BUNNY-BNB pool ratio is smaller than 5:1, this is profitable
    ...
    function valueOfAsset(address asset, uint256 amount) public view returns (uint valueInBNB) {
        ...
        if (IUniswapV2Pair(asset).token0() == BNB) {
            // multiply by 2 because each LP token represents 0.5 of each underlying
    ...
    function _zapAssetsToBunnyBNB(
        address asset, // USDC-BNB pair
        uint256 amount
    ) private returns (uint256 bunnyBNBAmount) {
        // takes balanceOf BUNNY-BNB LP tokens in this address
        ...
        if (IERC20(asset).allowance(address(this), address(router)) == 0) {
            // approve payment of BNB-USDC LP tokens to router
        ...
        // remove liquidity of 1% of BNB-USDC LP tokens
        ...
        // approve payment of BNB and USDC to zapBSC
        ...
        // swap BNB and USDC to LP tokens of BUNNY-BNB
