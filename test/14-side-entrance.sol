...
import { CallOptionsExploiter } from "src/side-entrance/CallOptionsExploiter.sol";
...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
       ...
       // USDC-DAI pair has 1:1 ratio
       // USDC-wETH pair has 2000:1 ratio
       // 1 ETH locked up, 2100 USDC strike, 100 USDC premium to purchase option, 1 week expiry of option
       // adminUser has approved USDC to optionsContract

        new CallOptionsExploiter(address(optionsContract), adminUser, address(uniRouter), address(uniFactory), address(usdc), address(usdcDaiPair), address(usdcEthPair), address(weth)).startExploit();
       // to execute the option, you have to buy it, sending premium to option owner, and you get 1 wETH
        vm.stopPrank();
        validation();
    }
