...
import { AssetExploiter } from "src/game-assets/AssetExploiter.sol";
...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
        ...
        // wrap the NFT, mint the wrapped, on MintFallback, unwrap the NFT
        // assetOwner is the address of the contract attacker || wrapper contract?

        new AssetExploiter(address(assetWrapper),address(swordAsset),address(shieldAsset)).exploit();
    ...
    function validation() public {
        ...
        // the NFT balances of the wrapper contract are 1 each
        assertEq(swordAsset.balanceOf(address(assetWrapper)),1);
        assertEq(shieldAsset.balanceOf(address(assetWrapper)),1);
        // the admin user is NOT the owner of the NFTs inside the wrapper contract
